...
void handle_page_fault(struct proc *p, uint64 fault_addr)
{
  // Check for valid virtual address
  int invalid = (fault_addr >= MAXVA);
    if (invalid) {
        p->killed = 1;
        return;
    }
  ...
  int is_cow = (*pte & PTE_COW);
    int is_shared = refcount[pa / PGSIZE] > 1;
  if ((is_cow))
  {
    if (is_shared)
    {
    ...
    if (f == 1)
    {
      ...
      uint64 new_flags = (PTE_FLAGS(*pte) | PTE_W) & ~PTE_COW;
      *pte = PA2PTE(newpage) | new_flags;
    ...
    {
      // Only one reference, mark the page as writable
